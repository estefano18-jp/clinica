...
<body>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            if (btnSiguienteComplementaria) {
                ...
                btnSiguienteComplementaria.addEventListener('click', function() {
                    // Validar solo contrato y horarios
                    if (validateContratoInfo() && validateHorarioInfo()) {
                        // Mostrar alerta de credenciales
            ...
            <div class="text-start">
                ...
                <div class="mb-3">
                    <label for="swal-nomuser" class="form-label text-start">Nombre de Usuario</label>
                    <div class="input-group">
                        <span class="input-group-text"><i class="fas fa-user"></i></span>
                        <input type="text" class="form-control" id="swal-nomuser" value="${emailDoctor}" disabled>
                        <button class="btn btn-outline-secondary" type="button" id="swal-editUsername">
                            <i class="fas fa-edit"></i> Editar
                        </button>
                    </div>
                    <div class="invalid-feedback" id="help-swal-nomuser"></div>
                </div>
                <div class="mb-3">
                    ...
                    <div class="invalid-feedback" id="help-swal-passuser"></div>
                </div>
            </div>
        `,
                    ...
                    didOpen: () => {
                        ...
                        // Configurar el botón de editar nombre de usuario
                        ...
                        if (passuser) document.getElementById('swal-passuser').value = passuser;

                        // Si el email está vacío, debería estar desbloqueado automáticamente
                        if (!emailDoctor) {
                            usernameInput.disabled = false;
                            editUsernameBtn.innerHTML = '<i class="fas fa-lock"></i> Bloquear';
                        }
                    },
                    preConfirm: () => {
                        // Validar los campos antes de cerrar la alerta
                        ...
                        if (!nomuser) {
                            ...
                            document.getElementById('help-swal-nomuser').textContent = 'El nombre de usuario es obligatorio';
                            document.getElementById('help-swal-nomuser').style.display = 'block';
                            isValid = false;
                        } else if (!/^[a-zA-Z0-9_@.-]{4,50}$/.test(nomuser)) {
                            document.getElementById('swal-nomuser').classList.add('is-invalid');
                            document.getElementById('help-swal-nomuser').textContent = 'El nombre de usuario debe tener entre 4 y 50 caracteres y puede incluir letras, números, _, @, . y -';
                            document.getElementById('help-swal-nomuser').style.display = 'block';
                            isValid = false;
                        } else {
                            document.getElementById('swal-nomuser').classList.remove('is-invalid');
                            document.getElementById('help-swal-nomuser').style.display = 'none';
                        }

                        // Validar contraseña
                        if (!passuser) {
                            document.getElementById('swal-passuser').classList.add('is-invalid');
                            document.getElementById('help-swal-passuser').textContent = 'La contraseña es obligatoria';
                            document.getElementById('help-swal-passuser').style.display = 'block';
                            isValid = false;
                        } else if (passuser.length < 6) {
                            document.getElementById('swal-passuser').classList.add('is-invalid');
                            document.getElementById('help-swal-passuser').textContent = 'La contraseña debe tener al menos 6 caracteres';
                            document.getElementById('help-swal-passuser').style.display = 'block';
                            isValid = false;
                        } else {
                            document.getElementById('swal-passuser').classList.remove('is-invalid');
                            document.getElementById('help-swal-passuser').style.display = 'none';
                        }

                        // Si no es válido, evitar que se cierre la alerta
                        if (!isValid) {
                            Swal.showValidationMessage('Por favor, complete correctamente todos los campos');
                            return false;
                        }

                        // Si es válido, guardar los valores en los campos ocultos del formulario
            ...
            function validateFechaNacimiento(fechaNacimiento) {
                ...
                // Verificar si es menor de edad - ESTE ES EL MENSAJE IMPORTANTE
                if (edad < 18) {
                    return {
                        isValid: false,
    ...
    <div class="row">
        ...
        `,
                                ...
                                fetch(`../../../controllers/persona.controller.php?op=buscar_por_documento&nrodoc=${nrodoc}`)
                                    ...
                                    .then(personaData => {
                                        ...
                                        if (personaData.status && personaData.persona) {
                                            // NUEVO CÓDIGO: Verificar edad antes de continuar
                                            // Verificar la edad de la persona
                                            ...
                                            // Si pasa la validación de edad, continuar con el proceso normal
            ...
            function registrarDoctorCompleto() {
                ...
                // Actualizar mensaje de carga con barra de progreso
                Swal.update({
                    title: 'Registrando...',
                    html: `
            <p class="mb-1">Registrando información personal y profesional...</p>
                ...
                fetch('../../../controllers/doctor.controller.php?op=registrar', {
                    ...
                    .then(data => {
                        ...
                        if (data.status) {
                            ...
                            Swal.update({
                                html: `
                    <p class="mb-1">Información personal y profesional registrada ✓</p>
                    <p class="mb-1">Registrando contrato...</p>
                    <div class="progress mb-3">
                        <div class="progress-bar progress-bar-striped progress-bar-animated" role="progressbar" 
                ...
                `
                    ...
                    .catch(error => {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Error de conexión',
                            text: 'No se pudo completar el registro del doctor: ' + error.message
                        });
                    });
            }
    ...
    `;
            ...
            function registrarHorarios(idcolaborador, idcontrato) {
                ...
                // Contar días seleccionados para mostrar progreso
                ...
                diasSemana.forEach((dia, index) => {
                    const atiende = document.getElementById(`atiende${dia}`);
                    if (atiende && atiende.checked) {
                        // Primero registrar la atención (día de la semana)
                        const formDataAtencion = new FormData();
                        formDataAtencion.append('idcontrato', idcontrato);
                        formDataAtencion.append('diasemana', diasDB[index]);

                        // Registrar la atención primero
                        fetch('../../../controllers/atencion.controller.php?op=registrar', {
                                method: 'POST',
                                body: formDataAtencion
                            })
                            .then(response => response.json())
                            .then(dataAtencion => {
                                console.log(`Respuesta registro atención ${dia}:`, dataAtencion);

                                if (dataAtencion.status && dataAtencion.idatencion) {
                                    const idatencion = dataAtencion.idatencion;
                                    const modalidad = document.getElementById(`modalidad${dia}`).value;

                                    // Registrar el primer horario (mañana)
                                    const horaInicio1 = document.getElementById(`horaInicio${dia}1`).value;
                                    const horaFin1 = document.getElementById(`horaFin${dia}1`).value;

                                    const formDataHorario1 = new FormData();
                                    formDataHorario1.append('idatencion', idatencion);
                                    formDataHorario1.append('horainicio', horaInicio1);
                                    formDataHorario1.append('horafin', horaFin1);

                                    // Registrar el primer horario
                                    return fetch('../../../controllers/horario.controller.php?op=registrar', {
                                            method: 'POST',
                                            body: formDataHorario1
                                        })
                                        .then(response => response.json())
                                        .then(dataHorario1 => {
                                            console.log(`Respuesta registro horario mañana ${dia}:`, dataHorario1);

                                            // Si es tiempo completo, registrar el segundo horario (tarde)
                                            if (modalidad === 'tiempoCompleto') {
                                                const horaInicio2 = document.getElementById(`horaInicio${dia}2`).value;
                                                const horaFin2 = document.getElementById(`horaFin${dia}2`).value;

                                                const formDataHorario2 = new FormData();
                                                formDataHorario2.append('idatencion', idatencion);
                                                formDataHorario2.append('horainicio', horaInicio2);
                                                formDataHorario2.append('horafin', horaFin2);

                                                // Registrar el segundo horario
                                                return fetch('../../../controllers/horario.controller.php?op=registrar', {
                                                    method: 'POST',
                                                    body: formDataHorario2
                                                });
                                            }
                                    ...
                                    throw new Error(`Error al registrar atención para ${dia}`);
                                }
                            })
                            .then(finalData => {
                                horariosExitosos++;
                                ...
                                console.log(`Horario de ${dia} registrado correctamente`);
                            })
                            .catch(error => {
                                console.error(`Error al registrar horario completo para ${dia}:`, error);
                                horariosConError.push(dia);
                            ...
                            .finally(() => {
                                // Si todos los horarios se han procesado, continuar con las credenciales
                                if (horariosExitosos >= horariosPendientes) {
                                    // Actualizar mensaje de carga
                                    Swal.update({
                                        title: 'Registrando...',
                                        html: 'Procesando credenciales'
            ...
            function registrarCredenciales(idcontrato, huboErroresHorarios = false) {
                ...
                Swal.update({
                    title: 'Registrando...',
                    html: 'Procesando credenciales de acceso'
                });
                        ...
                        setTimeout(() => {
                            ...
                            if (data.status) {
                                ...
                                if (huboErroresHorarios) {
                                    Swal.fire({
                                        icon: 'info',
                                        title: '¡Registro parcialmente completo!',
                                ...
                                // Error en credenciales pero el resto está registrado
                                Swal.fire({
                                    icon: 'warning',
                    ...
                    .catch(error => {
                        ...
                        setTimeout(() => {
                            ...
                            Swal.fire({
                                icon: 'warning',
                                title: 'Registro parcial',
                                html: 'La información del doctor y los horarios han sido registrados, pero hubo un problema de conexión al configurar las credenciales. <br><br>Puede configurar las credenciales más tarde desde la administración.',
                                confirmButtonText: 'Entendido'
                            }).then(() => {
                                resetFormAndRedirect();
                            });
